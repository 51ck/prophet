...
- Prompt + offer logic (T8.7 / T9.2): prefer fewer cards; `card-of-day` only when `sessionPath` is day-card; sharp hinge → `single-focus`; `beginRitual` / select spread only in `committed` (not again in `ritual` — would replace desk) — `assertCanSelectSpread` + phase/`sessionPath` lines in instructions; catalog ids via `CATALOG_SPREAD_IDS`
