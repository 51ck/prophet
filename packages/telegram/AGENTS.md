...
- Language change: clear switch phrases (`resolveLanguageChange`) persist via `updateProfile` before agent turn; no re-introduce; agent/prompt also handles via `updateSeekerProfile`
...
- Saved language + clear switch phrase → persist new language; do not re-ask name/self; later turns use new register via prompt
...
- `bun test packages/telegram` (format converter + parse-error fallback + ask keyboard/callback parse + free-text clears pending / typed decline + language gate + language change phrases + name/self introduce)
